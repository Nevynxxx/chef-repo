...
# Create the logo image referenced within mediawiki

cookbook_file "#{::File.join(app['deploy_to'], "current")}/skins/common/images/smslogosmall.gif" do
  owner app["owner"]
  group app["group"]
  mode "0755"
end

# Checkout the images Repo
directory "#{::File.join(app['deploy_to'], "shared/images")}" do
  action :create
  owner "images"
end
git "#{::File.join(app['deploy_to'], "shared/images")}" do
  repository "git@bitbucket.org:Nevynxxx/wiki-images.git"
  reference "master"
  user "images"
  action :sync
end

# Remove images dir that's been created
directory "#{::File.join(app['deploy_to'], "current/images")}" do
  recursive true
  action :delete
end

# Create link to new images dir
link "#{::File.join(app['deploy_to'], "current/images")}"  do
  to "#{::File.join(app['deploy_to'], "shared/images")}"
end
